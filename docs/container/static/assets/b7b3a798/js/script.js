$(function () {
    ...
    if (typeof accountId !== 'undefined' && accountId) {
        var localStorageKey = 'chromeExtension.id.' + accountId;
        var extItem = JSON.parse(localStorage.getItem(localStorageKey)) || {};
        ...
        if (cacheAbsentOrExpired) {
            extensionReady(function (extensionId) {
                $.post('/site/chrome-extension-installed?accountId=' + accountId, {isInstalled: Boolean(extensionId)})
                    .then(function (res) {
                        if (res === 'ok') {
